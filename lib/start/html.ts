...
const html = function start_html(complete_tasks:(task:type_start_primary_tasks) => void, process_path:string):core_start_task {
    return {
        label: "Read's the dashboard's HTML file for dynamic modification.",
        task: function start_html_task():void {
            const unwrap = function start_html_task_unwrap(fun:() => void):string {
                ...
                object = function start_html_task_object(store:store_function, features:boolean, first:string):string[] {
                ...
                complete = function start_html_task_complete(key:string):void {
                    flags[key] = true;
            ...
            if (vars.environment.features["terminal"] === true || vars.environment.features["compose-containers"] === true) {
                file.read({
                    callback: function start_html_task_readXtermCSS(file:Buffer):void {
                ...
                file.read({
                    callback: function start_html_task_readXtermJS(file:Buffer):void {
            ...
            if (vars.environment.features["statistics-resources"] === true) {
                file.read({
                    callback: function start_html_task_readChart(file:Buffer):void {
            ...
            // read styles.css
            file.read({
                callback: function start_html_task_readCSS(fileContents:Buffer):void {
