...
const test_summary = function test_summary(name:string, complete:boolean):void {
    ...
    if (complete === true) {
        ...
        summary.push(`    ${vars.text.angry}*${vars.text.none} Percentage assertion pass: ${vars.text[total_color] + pad_right(17, (((vars.test.total_assertions - vars.test.total_assertions_fail) / vars.test.total_assertions) * 100).toFixed(2)) + vars.text.none}%`);
    }
    ...
    if (complete === true && exit === true) {
        const item_service:services_test_browser = {
            ...
            socket:websocket_client = vars.data_store.sockets_tcp[vars.id.dashboard_server].open[0],
            payload:socket_data = {
                data: item_service,
